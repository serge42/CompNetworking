...
public class DNS implements Cloneable {
	...
	public void addAllAnswers(List<DNSResourceRecord> answers) {
	...
	public String toString() {
		...
		String result = String.format(
				...
				this.id, (this.query ? "Query" : "Response"), strOpcode, strRcode, this.authoritative, this.truncated,
	...
	public static String deserializeName(ByteBuffer bb) {
		...
		while ((labelLength > 0) || ((labelLength & 0b11000000) == 0b11000000)) {
			if ((labelLength & 0b11000000) == 0b11000000) {
				...
				bb.get(labelBytes);
				name += "." + new String(labelBytes, StandardCharsets.US_ASCII);
			}
			labelLength = bb.get();
		}
	...
	public static byte[] serializeName(String name) {
		// What if the name finishes by '.' -> should remove the +1
		// byte[] data = new byte[name.length() + 1 + (name.length() > 0 ? 1 : 0)];
		int nlength = name.length();
		byte[] data = new byte[nlength + (nlength > 0 && name.charAt(nlength - 1) == '.' ? 0 : 1)
				+ (nlength > 0 ? 1 : 0)];
		ByteBuffer bb = ByteBuffer.wrap(data);
